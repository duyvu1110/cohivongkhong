...
import numpy as np
from utils.data_gcn import load_data_GCN, build_collate_fn_GCN
from trainer.trainer_GCN import Trainer_GCN
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/home/qtxu/Sentiment_SPN2/data/Camera-COQE')
    parser.add_argument('--output_path', default='log/')
    parser.add_argument('--bert_directory', type=str, default="/home/qtxu/PLM/bert-base-uncased")
    ...
    collate_fn = build_collate_fn_GCN(args)
    data = {
        'train': DataLoader(load_data_GCN(args, 'train','train'), args.batch_size, True, collate_fn=collate_fn),
        'dev': DataLoader(load_data_GCN(args, 'dev', 'dev'), args.batch_size, False, collate_fn=collate_fn),
        'test': DataLoader(load_data_GCN(args, 'test', 'test'), args.batch_size, False, collate_fn=collate_fn),
    }
    model = SetPred4RE(args, 5).to(args.device)
    trainer = Trainer_GCN(model, data, args)
    trainer.train_model()
